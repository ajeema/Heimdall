from .strategy_planner import Planner
from .learning_modules import Researcher
from .data_formatting import Formatter
from .code_utils import Coder
from .actions import Action
from .thought_process import InternalMonologue
from .responses import Answer
from .execution_control import Runner
from .data_features import Feature
from .update_manager import Patcher
from .status_reporting import Reporter
...
from src.LanguageProcessing.sentence import SentenceBert
from src.Memory import KnowledgeBase
from src.WebInterface.search import BingSearch, GoogleSearch, DuckDuckGoSearch
from src.WebInterface import Browser
from src.WebInterface import start_interaction
from src.StorageSystem import ReadCode
from src.ServiceOrchestrator import Netlify
from src.DocumentationEngine.pdf import PDF
...
from src.NetworkSocket.socket_instance import emit_agent
...
class Agent:
    ...
    def make_decision(self, prompt: str, project_name: str) -> str:
        ...
        for item in decision:
            ...
            self.project_manager.add_message_from_heimdall(project_name, reply)
            ...
            if function == "git_clone":
                ...
                self.project_manager.add_message_from_heimdall(project_name, response)
    ...
    def execute(self, prompt: str, project_name_from_user: str = None) -> str:
        ...
        self.project_manager.add_message_from_heimdall(project_name, reply)
        self.project_manager.add_message_from_heimdall(project_name, json.dumps(plans, indent=4))
        # self.project_manager.add_message_from_heimdall(project_name, f"In summary: {summary}")
        ...
        if (queries and len(queries) > 0) or ask_user != "":
            self.project_manager.add_message_from_heimdall(
                project_name,
        ...
        if not queries and len(queries) == 0:
            self.project_manager.add_message_from_heimdall(project_name,
                                                           "I think I can proceed without searching the web.")

        ask_user_prompt = "Nothing from the user."

        if ask_user != "" and ask_user is not None:
            self.project_manager.add_message_from_heimdall(project_name, ask_user)
            self.agent_state.set_agent_active(project_name, False)
            ...
            while not got_user_query:
                ...
                if latest_message_from_user and validate_last_message_is_from_user:
                    ...
                    got_user_query = True
                    self.project_manager.add_message_from_heimdall(project_name, "Thanks! 🙌")
                time.sleep(5)
